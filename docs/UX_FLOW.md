...
```text
Настройки -> Start -> Раунд -> Отдых -> ... -> Финальный раунд -> Настройки
```
...
stateDiagram-v2
    [*] --> Setup
    Setup --> Round: Start / сигнал раунда
    Round --> Paused: Pause
    Rest --> Paused: Pause
...
Показываются `3` раунда, `3:00` раунд, `1:00` отдых и выключенное предупреждение. До первого Start приложение может запросить разрешение на уведомления после короткого объяснения пользы. Отказ не блокирует основной сценарий.

## 4. Активный таймер

### Общая иерархия

1. Текущий этап: Round или Rest.
...
- Round и Rest различаются подписью, контрастной цветовой схемой и звуком.
