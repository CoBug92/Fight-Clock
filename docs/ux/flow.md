...
Статус проверки: поток ниже сверен с текущими SwiftUI-экранами `SetupView`, `SoundSettingsView`, `ActiveTimerView`, `RootView` и Live Activity widget на 19 августа 2026 года.
...
Если статус разрешения ещё не определён, рядом показывается кнопка `Разрешить уведомления`. Пользователь может продолжить без перехода в Settings. После отказа карточка объясняет, что фоновые сигналы отключены и что нужно разрешить уведомления в настройках Fight Clock; нажатие на карточку открывает системные настройки приложения.
...
Пользовательские строки текущих экранов и Live Activity представлены в `Main/Resources/Localization/en.lproj/Localizable.strings` и `Main/Resources/Localization/ru.lproj/Localizable.strings`. Числовой формат таймера остаётся `M:SS`. Термины:
