...
final class NotificationScheduler: NotificationScheduling {
    ...
    func schedule(for state: SessionState, now: Date) async {
        ...
        for (index, boundary) in planner.futureBoundaries(from: state, after: now).enumerated() {
            ...
            let interval = max(.notificationSchedulerMinimumInterval, boundary.date.timeIntervalSince(now))
    ...
    func cancel(for state: SessionState) {
        let identifiers = (.zero...Self.maximumBoundaryCount).map {
    ...
    private func identifier(state: SessionState, index: Int) -> String {
        Self.identifierPrefix
            + state.id.uuidString
            + TechnicalString.period
            + state.notificationRevision.uuidString
            + TechnicalString.period
            + String(index)
    }
}
...
private extension TimeInterval {
    static let notificationSchedulerMinimumInterval: TimeInterval = 1
}
